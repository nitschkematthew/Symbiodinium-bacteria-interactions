...
Perform non-parametric Kruskal-Wallis test and dunnTest for post-hoc comparisons

```{r}
xeqK <- kruskal.test(xeq ~ strain, data = XB)
biofilmK <- kruskal.test(biofilm ~ strain, data = XB)

xeqD <- dunnTest(xeq ~ strain, data = XB)
biofilmD <- dunnTest(biofilm ~ strain, data = XB)
```
