...
Some non-elegant code for calculating the percentage of the reference-pathway completeness. Runs quickly but could be optimised to apply through pathko columns. All KOs of each pathway were retrieved from the reference pathway page at https://www.genome.jp/dbget-bin/www_bget?XXXXXXX
...
subp <- subset(pathko, select=c("ko00561","ko00564","ko04972","ko05111","ko02030","ko02060","ko04080","ko01054","ko00901","ko00300","ko00360","ko00906","ko00190","ko04961","ko01053","ko04115"))
KO <- colnames(subp)
...
d2 <- d %>% filter(str_detect(path, "Glycerolipid metabolism|Bacterial chemotaxis|Vibrio cholerae pathogenic cycle|Phosphotransferase|siderophore|Glycerophospholipid|Phenylalanine metabolism|Carotenoid"))
...
# Supplmentary pathway comparison plot

Plot significantly different pathways SP vs NP

```{r}
dplot <- merge(d, sigpath, by = "path")
  
p <- ggplot(data=dplot, aes(x=abund, y=path, colour = state)) +
  geom_point(size = 3) +
  geom_errorbarh(aes(xmin=abund-se, xmax=abund+se), height = 0.2) +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust=0.2)) +
...
# Calculate % completeness of interesting pathways

Pathway completeness calculated as above.
...
temp <- dfko[dfko$OTU %in% pathko$ko00500,]
  sel <- temp %>% select(OTU, Abundance, state, KO)
  l <- sel %>% group_by(OTU,KO,state) %>% summarize(abund = mean(Abundance))
  e <- length(which(pathko$ko00500 != ""))
  o <- nrow(l)
  ko00500 <- (o/e)*100
  
temp <- dfko[dfko$OTU %in% pathko$ko02040,]
  sel <- temp %>% select(OTU, Abundance, state, KO)
  l <- sel %>% group_by(OTU,KO,state) %>% summarize(abund = mean(Abundance))
  e <- length(which(pathko$ko02040 != ""))
  o <- nrow(l)
  ko02040 <- (o/e)*100
  
temp <- dfko[dfko$OTU %in% pathko$ko03070,]
  sel <- temp %>% select(OTU, Abundance, state, KO)
  l <- sel %>% group_by(OTU,KO,state) %>% summarize(abund = mean(Abundance))
  e <- length(which(pathko$ko03070 != ""))
  o <- nrow(l)
  ko03070 <- (o/e)*100

subp <- subset(pathko, select=c("ko00561","ko05111","ko02030","ko02060","ko01053","ko00500","ko02040","ko03070"))
KO <- colnames(subp)
completeness <- c(ko00561,ko05111,ko02030,ko02060,ko01053,ko00500,ko02040,ko03070)
PC <- data.frame(KO, completeness)
PC <- merge(PC, sigpathp, by = c("KO"))
View(PC)
```
...
```{r}
sigp <- left_join(sigpathp, d2, by = "path")

p <- ggplot(data=sigp, aes(x=abund, y=path, colour = phase)) +
  geom_point(size = 2) +
  geom_errorbarh(aes(xmin=abund-se, xmax=abund+se), height = 0.5) +
  theme(axis.text.y = element_text(angle = 0, hjust = 1, vjust=0.2)) +
  scale_color_manual(values =c(active = "green", post = "red", pre = "grey")) +
