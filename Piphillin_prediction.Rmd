...
## Testing commit from sourcetree
...
A function that melts the phyloseq object, filters the melted df to contain only the reference KOs, calculates mean and error of SP vs NP cultures, and returns the plot.

```{r}
pathwayplot <- function(phyloseqko, ID, refpaths){
  
  dfko <- psmelt(phyloseqko)

  biofilmko <- subset(refpaths, select = ID)
  colnames(biofilmko)[1] <- "ID"
  biofilmko$ID <- as.character(biofilmko$ID)
  dfko$OTU <- as.character(dfko$OTU)

dfko <- dfko[dfko$OTU %in% biofilmko$ID,]
...
```{r}
pathwayplot(SPNPko, ID = "ko00061", refpaths)
```
...
```{r}
pathwayplot(SPNPko, ID = "ko02025", refpaths)
```

.. and from the Vibrio biofilm model pathway (ko05111).

```{r}
pathwayplot(SPNPko, ID = "ko05111", refpaths)
```

.. and from the Escherichia coli biofilm formation pathway (ko02026).

```{r}
pathwayplot(SPNPko, ID = "ko02026", refpaths)
```

Plot orthologs from PTS pathway (ko02060).

```{r}
pathwayplot(SPNPko, ID = "ko02060", refpaths)
```

# Supplementary Differential abundance KOs
