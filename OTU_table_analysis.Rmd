---
title: "16S: Symbiodinium-bacteria interactions and functions supporting the formation of calcifying biofilms"
...
# Supplementary rarefaction curves
...
# Compare NP and SP sister strains 

146 vs RT146

```{r}
data <- transform_sample_counts(biom, function(x) x/sum(x))

df <- psmelt(data)
p.df <- df %>% filter(str_detect(Sample, "146|RT146")) %>%
    filter(Abundance > 0)

df146 <- p.df %>% filter(str_detect(state, "NP"))
dfRT146 <- p.df %>% filter(str_detect(state, "SP"))
unique146 <- setdiff(df146$OTU, dfRT146$OTU)
uniqueRT146 <- setdiff(dfRT146$OTU, df146$OTU)
both <- intersect(dfRT146$OTU, df146$OTU)

tax <- as.data.frame(tax_table(biom))
tax <- tibble::rownames_to_column(tax, var = "OTU")

OTU <- unique146
NP146 <- data.frame(OTU)
NP146 <- left_join(NP146, tax, by = "OTU")
View(NP146)

OTU <- uniqueRT146
SPRT146 <- data.frame(OTU)
SPRT146 <- data.frame(OTU)
SPRT146 <- left_join(SPRT146, tax, by = "OTU")
View(SPRT146)
```
...
plot_ordination(calctrans, brayordtrans, color="state", shape="state")  + 
...
plot_ordination(rare, brayordtransrare, color="state", shape="state") + 
  geom_text(aes(label = sample_names(rare)), vjust = 1.5) + 
  geom_point(size = 4)
```
...
ggplot(sigtabf, aes(x=Rank5, y=log2FoldChange, color=Rank5)) + 
                      geom_errorbar(aes(ymin = log2FoldChange + 
                      error, ymax = log2FoldChange - error), width = 0.2, colour = "black") + 
                      geom_point(size=5) + 
...
ggplot(sigtab, aes(x=Rank6, y=log2FoldChange, color=Rank6)) + 
  geom_errorbar(aes(ymin = log2FoldChange + error, ymax = log2FoldChange - error), width = 0.2, colour = "black") +
  geom_point(size=5) + 
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
```
...
beta3 <- betadisper(subtemptransbray, df$culture)
...
```{r echo=TRUE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
adonis(formula = subtemptransbray ~ phase, data = df)

adonis(formula = subtemptransbray ~ culture/phase, data = df, strata = df$culture)
